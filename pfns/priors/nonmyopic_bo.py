...
def get_batch(
    ...
    **kwargs,
    ...
    level = min(
        min(epoch // epochs_per_level, hyperparameters["max_level"]), num_evals - 1
    )
    if level_0_model:
        level = 1
        ...
        if considered_level > 0:
            ...
            for feed_x_b, feed_y_b in zip(
                ...
                with torch.cuda.amp.autocast():
                    style = (
                        torch.zeros(
                            feed_x_b.shape[1], 1, dtype=torch.int64, device=device
                        )
                        + considered_level
                        - 1
                    )
                ...
                if once_output and "mean_prediction" in once_output:
                    ...
                    assert (
                        ...
                        == (feed_x_b.shape[1], model.criterion.num_bars)
    ...
    return Batch(
        ...
        style=styles,
        mean_prediction=predict_mean_distribution.item()
