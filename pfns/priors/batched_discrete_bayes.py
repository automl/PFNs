...
class Stroke(DiscretePrior):
    def __init__(
        self, resolution=8, no_cut_off=False, noise_std=0.1, noise_type="normal"
    ):
        """
        :param weights: (num_latent_options, )
        """
    ...
    def sample_xy(
        self, num_samples=1, sample_among_first_n_variants=None, sample_half_half=False
    ):
        """
        Sample from p(x, y) num_samples times, from the latent options
        Return x: tensor, y: tensor
    ...
    for _ in range(batch_size):
        ...
        x = torch.stack(
            [
                ...
                shift_pixels(
                    prototypes[y[i]], translations_i[i].item(), translations_j[i].item()
                )
                for i in indices
            ]
        ).float()
...
class DiscreteBayes(torch.nn.Module):
    def __init__(
        self, priors: list[DiscretePrior], meta_weights: torch.Tensor | None = None
    ):
        """
