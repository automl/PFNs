...
class BarDistribution(nn.Module):
    def __init__(
        self, borders: torch.Tensor, smoothing=0.0, ignore_nan_targets=True
        ...
        assert (
            (1 - (full_width / (self.borders[-1] - self.borders[0]))).abs() < 1e-2
        ), f"diff: {full_width - (self.borders[-1] - self.borders[0])} with {full_width} {self.borders[-1]} {self.borders[0]}"
        assert (
    ...
    def mean_loss(self, logits, scaled_mean_logits):
        ...
        return (
            -scaled_mean_logits.gather(1, target_mean.T).mean(1).unsqueeze(0)
        )  # 1 x B
