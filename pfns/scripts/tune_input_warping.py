...
def log_vs_nonlog(x, w, *args, **kwargs):
    ...
    if true_nll:
        ...
        print(
            "true nll",
            fit_lbfgs(
                x, w, true_nll, **{**kwargs, "num_grad_steps": 0}, params0=best_params
            ),
        )
...
def fit_input_warping(
    model, x, y, nll_type="fast", old_solution=None, opt_method="lbfgs", **kwargs
):
    """
    ...
    def true_nll(x):  # noqa actually used with `eval` below
        ...
        for cutoff in range(len(x)):
            logits = model(
                x[:cutoff, None], y[:cutoff, None], x[cutoff : cutoff + 1, None]
            )
            total_nll = (
                total_nll
    ...
    def repeated_true_nll(x):  # noqa actually used with `eval` below
        ...
        for i in range(5):
            ...
            for cutoff in range(len(x)):
                logits = model(
                    x_[:cutoff, None], y_[:cutoff, None], x_[cutoff : cutoff + 1, None]
                )
                total_nll = (
                    total_nll
    ...
    def subset_nll(x):  # noqa actually used with `eval` below
        ...
        logits = model(
            train_x.transpose(0, 1), train_y.transpose(0, 1), eval_x.transpose(0, 1)
        )
