...
def test_move_to_larger():
    ...
    new_d = bar_distribution.BarDistribution(
        borders=torch.linspace(-1, 2, 1000)
    )
    new_logits = old_d.get_probs_for_different_borders(
        logits, new_d.borders
    ).log()
    assert old_d.median(logits).item() == approx(
        new_d.median(new_logits).item()
    )
    assert old_d.mean(logits).item() == approx(
...
def test_average_bar_distributions_into_different_one():
    ...
    bar_dists = [
        bar_distribution.BarDistribution(
            borders=torch.linspace(-1, -0.5, num_bars[0])
        ),
        bar_distribution.BarDistribution(
            borders=torch.linspace(0, 2, num_bars[1])
        ),
        bar_distribution.BarDistribution(
            borders=torch.linspace(1, 3, num_bars[2])
        ),
        bar_distribution.BarDistribution(
            borders=torch.linspace(2, 3, num_bars[3])
        ),
    ]

    new_d = bar_distribution.BarDistribution(
        borders=torch.linspace(-1, 2, 100)
    )
    ...
    new_small_d = bar_distribution.BarDistribution(
        borders=torch.linspace(-1, 2, 10)
    )
