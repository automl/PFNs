...
def get_model(x, y, hyperparameters):
    likelihood = gpytorch.likelihoods.GaussianLikelihood(
        noise_constraint=gpytorch.constraints.GreaterThan(1.0e-9)
    )
    model = ExactGPModel(x, y, likelihood)
    model.likelihood.noise = (
        torch.ones_like(model.likelihood.noise) * hyperparameters["noise"]
    )
    model.covar_module.outputscale = (
        torch.ones_like(model.covar_module.outputscale) * hyperparameters["outputscale"]
    )
    model.covar_module.base_kernel.lengthscale = (
        torch.ones_like(model.covar_module.base_kernel.lengthscale)
    ...
    with gpytorch.settings.fast_computations(
        *hyperparameters.get("fast_computations", (True, True, True))
        ...
        if equidistant_x:
            ...
            if hyperparameters.get("sampling", "uniform") == "uniform":
            ...
            elif isinstance(hyperparameters["sampling"], str) and hyperparameters[
                "sampling"
            ].startswith("uniform_"):
                left_border, right_border = [
                    float(v)
                    for v in hyperparameters["sampling"][len("uniform_") :].split("_")
                ]
                x = (
                ...
                dist_std, local_dist_std, base_likelihood = [
                    float(v)
                    for v in hyperparameters["sampling"][len("clustered_") :].split("_")
                ]
                ...
                def append_sample(samples, dist, local_dist, base_likelihood):
                    ...
                    sample_mean = torch.where(
                        use_base[:, None].repeat(1, num_features),
                        torch.zeros(batch_size, num_features),
                        samples[
                            torch.randint(num_samples, (batch_size,)),
                            torch.arange(batch_size),
                            :,
                        ],
                    )
                    return torch.cat(
                        (samples, (local_dist() + sample_mean).unsqueeze(0)), 0
                    )
                ...
                def create_sample(num_samples, dist, local_dist, base_likelihood):
                    ...
                    for i in range(num_samples):
                        samples = append_sample(
                            samples, dist, local_dist, base_likelihood
                        )
                ...
                x = (
                    create_sample(
                        seq_len,
                        lambda: torch.randn(batch_size, num_features) * dist_std,
                        lambda: torch.rand(batch_size, num_features) * local_dist_std,
                        base_likelihood,
                    )
                    .transpose(0, 1)
                    .to(device)
                ...
                blob_width, n_centers_max, stddev = [
                    float(v)
                    for v in hyperparameters["sampling"][len("gmix_") :].split("_")
                ]
                ...
                def get_x(
                    ...
                    centers = (
                        torch.rand((batch_size, n_centers, num_features), device=device)
                        * blob_width
                        - blob_width / 2
                    )
                    center_assignments = torch.randint(
                        n_centers,
                        (
                            batch_size,
                            n_samples,
                    ...
                    noise = (
                        torch.randn(
                            (batch_size, n_samples, num_features), device=device
                        )
                        * stddev
                    )
                    return (
                        centers.gather(
                            1, center_assignments[..., None].repeat(1, 1, num_features)
                        )
                        + noise
                    )  # centers: (b, m, f), ass: (b,n)
                ...
                (stddev,) = [
                    float(v)
                    for v in hyperparameters["sampling"][len("grep_") :].split("_")
                ]
                x = (
                    torch.randn(batch_size, seq_len // 2, num_features, device=device)
                    * stddev
                )
                x = x.repeat(1, 2, 1)
                ...
                ) * hyperparameters.get("sampling", 1.0)
            ...
            try:
                with gpytorch.settings.prior_mode(True):
                    ...
                    if observation_noise:
                        ...
                        target_sample = d.sample().transpose(
                            0, 1
                        )  # this will be the target for the loss
                        sample = likelihood(
                            target_sample
                        ).sample()  # this will be the input to the Transformer
        ...
        *hyperparameters.get("fast_computations", (True, True, True))
        ...
        for t in range(max(start_pos, 1), len(x), step_size):
            ...
            model, likelihood = get_model_on_device(
                x[:t].transpose(0, 1), y[:t].transpose(0, 1), hyperparameters, device
            )
        ...
        return (
            torch.stack(all_losses_after_t).to("cpu"),
...
if __name__ == "__main__":
    ...
    for redo_idx in range(1):
        print(
            evaluate(
                *get_batch(1000, 10, hyperparameters=hps, num_features=10),
                use_mse=False,
                hyperparameters=hps,
            )
        )
