...
from pfns.train import MainConfig, OptimizerConfig, BatchShapeSamplerConfig, TransformerConfig
from pfns.priors.hyperparameter_sampling import UniformFloatDistConfig, ChoiceDistConfig

from tqdm import tqdm

sampled_hp_prior = True
emsize=256
epochs=100
num_workers=6
lr=1e-4
...
def get_prior_config(sampled_hp_prior=sampled_hp_prior, plotting=False):
    ...
    if sampled_hp_prior:
        ...
        hyperparameters['outputscale_mean'] = UniformFloatDistConfig(.5, 3.)
        hyperparameters['lengthscale_std'] = UniformFloatDistConfig(.1, 1.5)
        hyperparameters['outputscale_std'] = UniformFloatDistConfig(.1, 3.)
        ...
        hyperparameters['hebo_noise_logmean'] = UniformFloatDistConfig(-8., -2.)
        hyperparameters['hebo_noise_std'] = UniformFloatDistConfig(.1, 5.)
...
config = MainConfig(
    ...
    model=TransformerConfig(
        ...
        nhid=emsize*4,
        nlayers=8,
        encoder=EncoderConfig(variable_num_features_normalization=True,
                            constant_normalization_mean=.5, constant_normalization_std=1/math.sqrt(12)),
        y_encoder=EncoderConfig(nan_handling=True),
        attention_between_features=True,
        style_encoder=StyleEncoderConfig(normalize_to_hyperparameters=hps) if sampled_hp_prior else None,
        y_style_encoder=StyleEncoderConfig(normalize_to_hyperparameters=hps) if sampled_hp_prior else None,
    ),
    batch_shape_sampler=BatchShapeSamplerConfig(batch_size=32, max_seq_len=60, fixed_num_test_instances=10, max_num_features=18),
    epochs=epochs,
