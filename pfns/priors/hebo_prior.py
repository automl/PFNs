...
import gpytorch
import torch
from botorch.models.transforms.input import *  # noqa: F403
...
def get_model(x, y, hyperparameters: dict, sample=True):
    ...
    model = botorch.models.SingleTaskGP(
        x, y, likelihood, covar_module=covar_module, input_transform=warp_tf
    )
        ...
        *hyperparameters.get("fast_computations", (True, True, True))
        ...
        for i in range(0, total_num_candidates, num_candidates):
            ...
            get_model_and_likelihood = lambda: get_model(
                used_local_x,
                torch.zeros(num_candidates, x.shape[1], device=device),
                hyperparameters,
            )
