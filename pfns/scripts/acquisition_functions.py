...
def log01_batch(x, eps=0.0000001, input_between_zero_and_one=False):
    ...
    for b in range(x.shape[-1]):
        x[:, b, b] = log01(
            x[:, b, b], eps=eps, input_between_zero_and_one=input_between_zero_and_one
        )
    return x
...
def general_power_transform(x_train, x_apply, eps, less_safe=False):
    if eps > 0:
        ...
        try:
            ...
            pt.fit(x_train.cpu().double())
        x_out = torch.tensor(
            pt.transform(x_apply.cpu()), dtype=x_apply.dtype, device=x_apply.device
        )
    ...
    for x_feed in torch.split(x_predict, max_dataset_size, dim=0):
        ...
        if ensemble_log_dims == "01":
            ...
            x_full_feed = torch.cat(
                (log01_batch(x_full_feed)[:, :-1], log01_batch(1.0 - x_full_feed)), 1
            )
...
def optimize_acq_w_lbfgs(
    ...
    **kwargs,
    ...
    if len(known_x) < pre_sample_size:
        x_initial = torch.cat(
            (rand_sample_func(pre_sample_size - len(known_x)).to(device), known_x), 0
        )
    else:
    ...
    def opt_f(x):
        ...
        acq = general_acq_function(
            model, known_x, known_y, x_eval_new, return_actual_ei=True, **kwargs
        )
        ...
        return neg_mean_acq.detach().cpu().to(
            torch.float64
        ).numpy(), x_eval.grad.detach().view(*x.shape).cpu().to(torch.float64).numpy()
