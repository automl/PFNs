...
from pfns.model import transformer, encoders
from pfns.model.transformer import PerFeatureTransformer, DEFAULT_EMSIZE, isolate_torch_rng
...
def test_feature_positional_embeddings(sample_data):
    ...
    embedding_types = ["normal_rand_vec", "uni_rand_vec", "learned", "subspace", None]
...
def test_decoder_dict(sample_data):
    ...
    assert outputs["standard"].shape == (sample_data["seq_len_test"], sample_data["batch_size"], 1)
    assert outputs["custom"].shape == (sample_data["seq_len_test"], sample_data["batch_size"], 2)
