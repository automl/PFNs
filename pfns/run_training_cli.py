    ...
    try:
        ...
        try:
            ...
            if not hasattr(config_module, 'config'):
                raise AttributeError(f"Config file {config_file} must define a 'config' variable")
    ...
    try:
        ...
        if args.checkpoint_path is not None:
            config = config.__class__(
                **{**config.__dict__, 'train_state_dict_save_path': args.checkpoint_path}
            )
        
        if args.load_checkpoint is not None:
            config = config.__class__(
                **{**config.__dict__, 'train_state_dict_load_path': args.load_checkpoint}
            )
